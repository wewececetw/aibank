            ...
            <div class="btn_bottom_nomargin">
                ...
                <div class="t100">
                    <a href="/front/myaccount_not_paid">
                        <div class="member_btn list_active">未繳款</div>
                    </a>
                </div>
                <div class="t100">
...
?>
                <div style="line-height: 45px;float: right;">未繳款總計：<span><?=number_format($all)?></span></div>
                {{-- 查看投標流程 --}}
<? /*?>
                ...
                <table id="keywords" cellspacing="0" cellpadding="0" class="rwd-table tablesorter t_color ">
                    ...
                    <tbody>
                        ...
                        <tr name="1" class="tender_document_detail">
                            <td class="lalign bd-example-modal-lg showmd rwd_td" data-th="債權憑證號"  data-risk="{{ $v['risk_category'] }}" data-id="{{ $v['claim_id'] }}">
                            ...
                            <td class="bd-example-modal-lg rwd_td" data-th="收益明細">
                                {{-- <div class="center"> --}}
                                    <button type="button" class="btn no-btn-style btn_nobg detailBtn" data-claimId="{{ $v['claim_id'] }}" data-amount="{{ $v['amount'] }}" data-tender_id="{{ $v['tender_documents_id'] }}" data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> Processing Order">
                                        <i class="fa fa-eye dt{{$v['tender_documents_id']}}" aria-hidden="true"></i>
                                    </button>

                                {{-- </div> --}}
                            </td>
                            <td class=" rwd_td" data-th="債權憑證">
                                {{-- <div class="center"> --}}
...
function category_modal(target){
    $.ajax({
        type:"POST",
        url:"/front/m_get_claims_html/"+target,
        dataType:"json",
        data:{
            id:target,
        }
...
<script type="text/javascript">
    ...
    $(".detailBtn").click(function(){
        ...
        tender_modal(tender_id,claim_id,amount);
    })
    ...
    function tender_modal(id,claim_id,amount) {
        gloid = id;
        btnloadingIcon(id, 'show');

        $("#detail_tbd").empty();
        setModal(id,claim_id,amount);
    ...
    function setModal(id,claim_id,amount) {
        let am='';
        ...
        $.ajax({
            ...
            success: function (d) {
                if (d.status == 'success') {
                    appendDataToModal(d.data,d.total);
    ...
    function appendDataToModal(data,total) {
        let x = 0,
        ...
        <tr>
            ...
            <td data-th="返還淨值">${  data.totala[x] }</td>
            <td data-th="入帳日"></td>
            <td data-th="返還日"></td>
        </tr>
        `;
        // <td data-th="手續費">${Math.round(fee)}</td>
        ...
        <tr>
            ...
            <td data-th="還投資金額">${  total.everyMonthPrincipal }</td>
            <td data-th="利潤"><span class="fbold">${  total.everyMonthInterest }</span> </td>
            <td data-th="手續費">${ total.thirdPartyManagmentFee }</td>
            <td data-th="總應返還"><span class="fbold">${  total.everyMonthPaidTotal }</span></td>
            <td data-th="返還淨值">${  total.totala }</td>
            <td data-th="入帳日"></td>
            <td data-th="返還日"></td>
        </tr>    
        `;
