...
@section('content')
<?
    $l1=" onclick=window.location=&#39;/front/claim_category/".$line_id."/";
    $l2="&#39;";
    $rr[0]="先息後本";
    $rr[1]="本息攤還";
?>
    ...
    <div class="container">
               ...
               <div class="btn_left">
                ...
                <div id="invest" class="btn_right_R">
                    <div class="btn_3 active_2">
                        ...
                        <a id="send_tender" href="/front/claim_match" style="cursor: pointer;">
                            <i class="fa fa-cog" aria-hidden="true"></i>快速媒合
                        </a>
                    </div>
                </div>
            </div>
            <table id="claims" style="width:100%;" class="table table-striped table-bordered rwd-table tablesorter tablesorter-default " >
                <thead>
                    <tr class="title_tr">
                ...
                <tbody>
                    ...
                    @empty
                        <tr>
                            <td colspan="10">
                                <span class="fbold">--目前尚未有上架債權--</span>
                            </td>
                        </tr>
                    @endforelse
        ...
        <ul class="pagination" id='page1'>
                ...
                <li class="paginate_button page-item <? if($line_id==1){ ?>active<? }?>">
                    <a href="/front/claim_category/1/{{$c_t}}" aria-controls="claims" data-dt-idx="1" tabindex="0" class="page-link">1</a>
                </li>
<?
if($all_page>5){
        $ll = $line_id -3;
        ...
        for($ll;$ll<=$i;$ll++){
            if($ll > 1){
                ?><li class="paginate_button page-item <? if($ll==$line_id){ ?>active<? }?>">
                    <a href="/front/claim_category/{{$ll}}/{{$c_t}}" aria-controls="claims" data-dt-idx="{{$ll}}" tabindex="0" class="page-link">{{$ll}}</a>
                </li><?
            }
        }
        if($i < $all_page -1){
...
?>
                <li class="paginate_button page-item <? if($ll==$line_id){ ?>active<? }?>">
                    <a href="/front/claim_category/{{$all_page}}/{{$c_t}}" aria-controls="claims" data-dt-idx="{{$all_page}}" tabindex="0" class="page-link">{{$all_page}}</a>
                </li>
<? }?>
            ...
            <ul class="pagination" id='page2'>
                ...
                <li class="paginate_button page-item <? if($line_id==1){ ?>active<? }?>">
                    <a href="/front/claim_category/1/{{$c_t}}" aria-controls="claims" data-dt-idx="1" tabindex="0" class="page-link">第一頁</a>
                </li>
                <li class="paginate_button page-item previous {{$pre}}">
                ...
                <li class="paginate_button page-item <? if($ll==$line_id){ ?>active<? }?>">
                    <a href="/front/claim_category/{{$all_page}}/{{$c_t}}" aria-controls="claims" data-dt-idx="{{$all_page}}" tabindex="0" class="page-link">最末頁</a>
                </li>
            </ul>   
        </div>
    </div>
...
<!-- modal -->
<div class="modal fade show" id="claim_de"  style="padding-right: 17px; display: none;background: rgba(0, 0, 0, 0.5);;overflow-y: scroll;">
</div>
<!-- modal    -->
</div>
