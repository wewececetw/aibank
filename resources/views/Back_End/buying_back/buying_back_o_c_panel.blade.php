            ...
            <table id="claims_table" class="table table-bordered table_breaking m-b-10">
                <thead>
                    <tr>
                        ...
                        <th class="all">買回按鈕</th>
                        <th class="all">會計對帳按鈕</th>
                        <th class="all">銀行報表按鈕</th>
...
<script>
    $(document).ready(function () {
        ...
        function trTemp(d){
            ...
            <tr>
                ...
                <td>${d.sty_type_bt1}</td>
                <td>${d.sty_type_bt2}</td>
                <td>${d.sty_type_bt3}</td>
    ...
    function buy_back(claim_id){
        var _token="{{csrf_token()}}";
        if( password = prompt('請輸入買回密碼')){

            t_g_r_d_input = document.getElementById("t_g_r_d_input"+claim_id).value;
            p_d_input = document.getElementById("p_d_input"+claim_id).value;
            b_b_type_input = document.getElementById("b_b_type_input"+claim_id).value;

            if(b_b_type_input==1 || b_b_type_input==2){
                // alert(_token);
                $.post("/admin/buying_back_o_c_post",{claim_id,password,t_g_r_d_input,p_d_input,b_b_type_input,_token},function(oo){
                    alert(oo);
                });
            }else{
                alert("請選擇買回類型");
    ...
    function buy_back_ex1(claim_id){

        x1 = document.getElementById("t_g_r_d_input"+claim_id).value;
        x2 = document.getElementById("p_d_input"+claim_id).value;
        x3 = document.getElementById("b_b_type_input"+claim_id).value;
        ...
        if( x1.length > 0 && x2.length > 0 && x3.length > 0 ){
            ...
            let url = "{{ url('/admin/buying_back_ex1_get') }}" + "?" + $.param(data);
            window.location = url;
        }else{
            if(x1.length == 0){
                alert('預定入帳日不可為空');
    ...
    function buy_back_ex2(claim_id){
        ...
        if( x1.length > 0 && x2.length > 0 && x3.length > 0 ){
            ...
            let url = "{{ url('/admin/buying_back_ex2_get') }}" + "?" + $.param(data);
            window.location = url;
        }else{
            if(x1.length == 0){
                alert('預定入帳日不可為空');
