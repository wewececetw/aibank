            ...
            <div style="border:solid 1px #1a2732">
                ...
                <div class="panel-body">
                    <form class="simple_form new_match_performance" id="insert_claim_form" enctype="multipart/form-data"
                        ...
                        @if(isset($editClaims))
                        <input type="text" name="claim_id" value="{{ $editClaims }}" hidden>
                        @endif
                        <div class="detail_1" style="margin-bottom:40px;">
                            ...
                            <div class="row m-b-15">
                                <div class="col-sm-6">
                                    ...
                                    <input type="text" maxlength="8" name="tax_id" class="form-control"
                                        id="tax_id">
                                </div>
                            ...
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td>
                                            <input type="text" id="weekly_time"
                                                class="datepicker form-control datepicker_style" name="weekly_time">
                                        </td>
                                        {{-- <td>
                                            下標後繳款期限
                                        </td> --}}
                            ...
                            <div class="col-sm-12" style="margin:20px auto;">
                                ...
                                <button id="save_btn" type="button" onclick="insert_item();"
                            ...
                            <div class="row" id="pdf_block">
                                @if(isset($claimPdf))
                                <div id="myCarousel" class="carousel slide" data-ride="carousel" style="width:50%;margin-left: auto;
                                margin-right: auto;">
                                    <!-- Indicators -->
                                    <ol class="carousel-indicators">
                                        <?php $x=0; ?>
                                        @foreach ($claimPdf as $file)
                                        @if($x == 0)
                                        ...
                                        <div class="item active">
                                            ...
                                            <div class="item">
                                                ...
                                                <div class="carousel-caption">
                                                    ...
                                                    <p>
                                                        <a class="btn btn-danger removePdf"
                                                            data-id="{{ $file->claim_files_id }}">
                                                            刪除
                                                        </a>
                                                    </p>
                                                </div>
                                    ...
                                    <div class="caption">
                                        ...
                                        <p style="text-align: center">
                                            <a class="btn btn-danger removePdf" data-id="{{ $file->claim_files_id }}">
                                                刪除
                                            </a>
                                        </p>
                                    </div>
                                ...
                                <input type="text" name="removePdf" id="removePdf" hidden>
                                @endif
                                ...
                                <div class="col-sm-12">
                                    <div style="text-align: center">
                                        <img class="ui fluid image" id="preview-0" src="#" alt="" hidden
                                            style="max-width:50%">
                                    </div>
                                    <small>圖片大小上限:{{ config('fileSizeLimit.claims_create_pdf_max_size') }}MB</small>
                                    <input class="form-control pdfInput" name="pdf_name[]" id="pdf_name" type="file"
                                        placeholder="請選擇JPG、PNG檔" onchange="readURL(this)" data-type="0" multiple>
                                    <label for="files">(請選擇JPG、PNG檔)</label>
                                </div>
                            </div>
                        </div>
                    </form>
...
<script>
    function readURL(input) {
        if (input.files && input.files[0]) {
            ...
            reader.onload = function (e) {
                $('#preview-' + $(input).data('type')).attr('src', e.target.result);
                $('#preview-' + $(input).data('type')).show();
    ...
    function addPdf() {
        ...
        // <div class="col-sm-12">
        //     <small>圖片大小上限:{{ config('fileSizeLimit.claims_create_pdf_max_size') }}MB</small>
        //     <input class="form-control pdf pdfInput" name="pdf_name[]" type="file" id="pdf_${pdfCount}"
        //         placeholder="請選擇JPG、PNG檔">
        //     <label for="files">(請選擇JPG、PNG檔)</label>
        // </div>
        // `;
        let tmp = `
        <div class="col-sm-12">
            <div style="text-align: center">
                <img class="ui fluid image" id="preview-${pdfCount}" src="#" alt="" hidden style="max-width:50%">
            </div>
            <small>圖片大小上限:{{ config('fileSizeLimit.claims_create_pdf_max_size') }}MB</small>
            <input class="form-control pdfInput pdf" name="pdf_name[]" id="pdf_${pdfCount}" type="file"
                placeholder="請選擇JPG、PNG檔" onchange="readURL(this)" data-type="${pdfCount}">
            <label for="files">(請選擇JPG、PNG檔)</label>
        </div>
        `;
    ...
    function removePdf() {
        ...
        let lastChild = $("#pdf_block>div:last-child");
        if ($("#pdf_block>div:last-child > input").hasClass('pdf')) {
            $("#pdf_block>div:last-child").remove();
        }
    }
    //新增PDF 點擊事件listener
    ...
    function pdfInputListener() {
        $('.pdfInput').unbind().on('change', function () {
            ...
            if (!isPDF(file.val())) {
                ...
                return failValidation('您所選擇上傳的檔案格式有誤，請選擇JPG、PNG檔進行上傳');
            }
            ...
            if (fileSize > "{{ config('fileSizeLimit.claims_create_pdf_max_size') }}") {
                something_happens($(this));
                let sysFileSizeMax = "{{ config('fileSizeLimit.claims_create_pdf_max_size') }}MB";
                return failValidation('您所選擇上傳的檔案大小超過上限<br>您輸入的大小為:' + (Math.round(fileSize * 100) / 100) +
                    'MB<br>系統上限為:' + sysFileSizeMax);
            }
        });
    }
    ...
    function insert_item() {
        ...
        if ($("#insert_claim_form").valid()) {
            $.ajax({
                ...
                error: function (e) {
                    if (e.status == 422) {
                        let errors = JSON.parse(e.responseText).errors;
                        let li = '';
...
<script>
    // $(function () {
        ...
        $.ajax({
            url: '{{ url("/admin/claim_data/$editClaims") }}',
            type: 'get',
            success: function (d) {
                changeEditValue(d);
            },
        ...
        function setSpecialValue(key, v) {
            switch (key) {
                case 'risk_category':
                ...
                case 'foreign_t':
                case 'loan_type':
                case 'cooperative_company_id':
                    var opt = $("#" + key + " option:contains(" + v + ")");
                    $("#" + key).val($(opt).val());
                    break;
                case 'is_auto_closed':
                case 'is_pre_invest':
                    if (v == 1) {
                        $("input[name='" + key + "']").prop('checked', true).click();
                    }
                    break;
                case 'description':
                    CKEDITOR.instances['description'].setData(v)
        ...
        $(".removePdf").click(function () {
            var claimFileId = $(this).attr('data-id');
            var nowVal = $("#removePdf").val().split(',');
            if (nowVal.indexOf(claimFileId) == -1) {
                nowVal.push(claimFileId);
                $("#removePdf").val(nowVal.toString());
                changeCloseBtn($(this), true);
            } else {
                nowVal.splice(nowVal.indexOf(claimFileId), 1);
                $("#removePdf").val(nowVal.toString());
                changeCloseBtn($(this), false);
            }
        })
    };


    function changeCloseBtn(btn, trig) {
        if (trig == true) {
            btn.text('恢復');
