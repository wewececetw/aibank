...
$(function() {
    ...
    // 列表頁的 datatables 設定
    $('.role_permissions_table').each(function(){
        var table = $(this);
        var tab_key = table.data('tab_key');

        var editor = new $.fn.dataTable.Editor({
            ajax: {
                url: '/admin/role_permissions/update_row',
                type: 'PATCH'
            },
            table: "#"+ tab_key +"_role_permissions_table",
            idSrc: "DT_RowId",
            fields: [
                {
                fieldInfo: '',
                label: 'role_id',
                label: I18n.t('activerecord.attributes.role_permission.role_id'),
                name: 'role_id',
            },
            {
                fieldInfo: '',
                label: 'permission_id',
                label: I18n.t('activerecord.attributes.role_permission.permission_id'),
                name: 'permission_id',
            },
            ],
        });
        ...
        var datatable = table.DataTable({
            ...
            dom: 'Brtip"bottom"l',
            ajax: {
                url: '/admin/role_permissions/datatables.json',
                dataSrc: 'data',
                type: 'POST',
                data: {
                },
            ...
            columns:[
                {
                data: 'role_id',
                name: 'role_id_between',
                visible: true,
                orderable: true,
                className: 'editable col-role_id',
            editField: 'role_id',
            },
            {
                data: 'permission_id',
                name: 'permission_id_between',
                visible: true,
                orderable: true,
                className: 'editable col-permission_id',
            editField: 'permission_id',
            },
                {
                    // 操作
                    data: null,
                    ...
                    render: function ( data, type, full, meta ) {
                        ...
                        var actions = '<a href="/admin/role_permissions/' + id + '?master_show_tab='+ master_show_tab_value +'" class="btn btn-default m-r-5"><span class="glyphicon glyphicon-eye-open"></span></a>';
                        actions += '<a href="/admin/role_permissions/' + id + '/edit?master_show_tab='+ master_show_tab_value +'" class="btn btn-default m-r-5"><span class="glyphicon glyphicon-pencil"></span></a>';
                        actions += '<a href="/admin/role_permissions/' + id + '" data-confirm="確定刪除?" class="btn btn-default" rel="nofollow" data-method="delete"><span class="glyphicon glyphicon-trash"></span></a>';
                        return actions;
                    },
                },
            ],
        ...
        $("#"+ tab_key +"_role_permissions_table").on( 'click', 'tbody td.editable', function (e) {
        ...
        datatable.on('row-reorder', function(e, diff, edit){
            ...
            $.ajax({
                type: 'patch',
                url: '/admin/role_permissions/update_row_sorting',
                data: data,
                datatype: 'json'
            });
        });
    ...
    // 點擊頁籤
    $('.role_permission__tab').on('click', function(){
        var tab = $(this);
