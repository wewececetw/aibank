...
$(function() {
    ...
    // 列表頁的 datatables 設定
    $('.action_items_table').each(function(){
        var table = $(this);
        var tab_key = table.data('tab_key');

        var editor = new $.fn.dataTable.Editor({
            ajax: {
                url: '/admin/action_items/update_row',
                type: 'PATCH'
            },
            table: "#"+ tab_key +"_action_items_table",
            idSrc: "DT_RowId",
            fields: [
                {
                    fieldInfo: '',
                    label: 'name',
                    label: I18n.t('activerecord.attributes.action_item.name'),
                    name: 'name',
                },
                {
                    fieldInfo: '',
                    label: 'description',
                    label: I18n.t('activerecord.attributes.action_item.description'),
                    name: 'description',
                },
            ],
        });
        ...
        var datatable = table.DataTable({
            ...
            dom: 'Brtip"bottom"l',
            ajax: {
                url: '/admin/action_items/datatables.json',
                dataSrc: 'data',
                type: 'POST',
                data: {
                },
            ...
            columns:[
                ...
                },
                {
                    ...
                    render: function ( data, type, full, meta ) {
                        ...
                        var actions = '<a href="/admin/action_items/' + id + '?master_show_tab='+ master_show_tab_value +'" class="btn btn-default m-r-5"><span class="glyphicon glyphicon-eye-open"></span></a>';
                        actions += '<a href="/admin/action_items/' + id + '/edit?master_show_tab='+ master_show_tab_value +'" class="btn btn-default m-r-5"><span class="glyphicon glyphicon-pencil"></span></a>';
                        actions += '<a href="/admin/action_items/' + id + '" data-confirm="確定刪除?" class="btn btn-default" rel="nofollow" data-method="delete"><span class="glyphicon glyphicon-trash"></span></a>';
                        return actions;
                    },
                },
            ],
        ...
        $("#"+ tab_key +"_action_items_table").on( 'click', 'tbody td.editable', function (e) {
        ...
        datatable.on('row-reorder', function(e, diff, edit){
            ...
            $.ajax({
                type: 'patch',
                url: '/admin/action_items/update_row_sorting',
                data: data,
                datatype: 'json'
            });
        });
    ...
    // 點擊頁籤
    $('.action_item__tab').on('click', function(){
        var tab = $(this);
