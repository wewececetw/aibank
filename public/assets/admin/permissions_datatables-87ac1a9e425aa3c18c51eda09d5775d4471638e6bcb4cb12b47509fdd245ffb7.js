...
$(function() {
    ...
    // 列表頁的 datatables 設定
    $('.permissions_table').each(function(){
        var table = $(this);
        var tab_key = table.data('tab_key');

        var editor = new $.fn.dataTable.Editor({
            ajax: {
                url: '/admin/permissions/update_row',
                type: 'PATCH'
            },
            table: "#"+ tab_key +"_permissions_table",
            idSrc: "DT_RowId",
            fields: [
                {
                    fieldInfo: '',
                    label: 'name',
                    label: I18n.t('activerecord.attributes.permission.name'),
                    name: 'name',
                },
            ],
        });
        ...
        var datatable = table.DataTable({
            ...
            dom: 'Brtip"bottom"l',
            ajax: {
                url: '/admin/permissions/datatables.json',
                dataSrc: 'data',
                type: 'POST',
                data: {
                },
            ...
            columns:[
                {
                    data: 'resource_item',
                    name: 'resource_item_id_eq',
                    visible: true,
                    orderable: true,
                    className: 'col-resource_item_id',
                    editField: 'resource_item_id',
                },
                {
                    data: 'action_item',
                    name: 'action_item_id_eq',
                    visible: true,
                    orderable: true,
                    className: 'col-action_item_id',
                    editField: 'action_item_id',
                },
                {
                    data: 'name',
                    name: 'name_cont',
                ...
                },
                {
                    ...
                    render: function ( data, type, full, meta ) {
                        ...
                        var actions = '<a href="/admin/permissions/' + id + '" data-confirm="確定刪除?" class="btn btn-default" rel="nofollow" data-method="delete"><span class="glyphicon glyphicon-trash"></span></a>';
                        return actions;
                    },
                },
            ],
        ...
        $("#"+ tab_key +"_permissions_table").on( 'click', 'tbody td.editable', function (e) {
        ...
        datatable.on('row-reorder', function(e, diff, edit){
            ...
            $.ajax({
                type: 'patch',
                url: '/admin/permissions/update_row_sorting',
                data: data,
                datatype: 'json'
            });
        });
    ...
    // 點擊頁籤
    $('.permission__tab').on('click', function(){
        var tab = $(this);
